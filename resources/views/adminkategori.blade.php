                    ...
                    <div class="xl:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                        ...
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-3">
                                                    <div
                                                        class="w-11 h-11 rounded-xl bg-emerald-100 text-emerald-800 flex items-center justify-center">
                                                        <span
                                                            class="material-symbols-outlined">{{ $category->icon ?? 'category' }}</span>
                                                    </div>
                                                    <div>
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="flex justify-end gap-1">
                                                    <button
                                                        class="btn-edit p-2 text-outline hover:text-primary hover:bg-emerald-50 rounded-lg transition-colors"
                                                        title="Edit"
                                                        data-id="{{ $category->id }}"
                                                        data-nama="{{ $category->nama }}"
                                                        data-slug="{{ $category->slug }}"
                                                        data-icon="{{ $category->icon }}"
                                                        data-urutan="{{ $category->urutan }}"
                                                        data-deskripsi="{{ $category->deskripsi }}"
                                                        data-is_active="{{ $category->is_active }}">
                                                        <span class="material-symbols-outlined text-[20px]">edit</span>
                                                    </button>
                                                    <form action="{{ route('admin.kategori.destroy', $category->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori ini?')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button
                                                            type="submit"
                     ...
                     <aside class="bg-white rounded-xl shadow-sm border border-slate-200 p-xl h-fit">
                        <div class="flex items-center justify-between mb-lg">
                            <div>
                                <h2 id="form-title" class="font-h3 text-lg text-slate-900">Form Kategori</h2>
                                <p class="font-body-sm text-outline">Tambah atau ubah data kategori.</p>
                            </div>
                            <span class="material-symbols-outlined text-primary">edit_square</span>
                        </div>
                        <form id="kategori-form" action="{{ route('admin.kategori.store') }}" method="POST" class="space-y-md">
                            @csrf
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Contoh: Produk Rumah" type="text" required />
                            </div>
                            <div>
                                <label
                                ...
                                <input
                                    ...
                                    value="{{ old('slug') }}"
                                    class="w-full p-md border border-slate-300 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"
                                    placeholder="produk-rumah" type="text" required />
                            </div>
                            <div class="grid grid-cols-2 gap-md">
                                <div>
                                    ...
                                    <select
                                        ...
                                        <option value="category" {{ old('icon') == 'category' ? 'selected' : '' }}>category</option>
                                        <option value="restaurant" {{ old('icon') == 'restaurant' ? 'selected' : '' }}>restaurant</option>
                                        <option value="styler" {{ old('icon') == 'styler' ? 'selected' : '' }}>styler</option>
                                        <option value="handyman" {{ old('icon') == 'handyman' ? 'selected' : '' }}>handyman</option>
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="Deskripsi singkat kategori..." rows="4">{{ old('deskripsi') }}</textarea>
                            </div>
                            <label
                                ...
                                <input name="is_active" value="1" class="rounded border-slate-300 text-primary focus:ring-primary" {{ old('is_active', '1') == '1' ? 'checked' : '' }}
                                    type="checkbox" />
                            </label>
                            <div class="flex gap-3">
                                <button
                                    id="btn-submit"
                                    class="flex-1 py-3 bg-primary text-on-primary rounded-xl font-button hover:bg-primary-container transition-colors"
                                    type="submit">
                                    Simpan Kategori
                                </button>
                                <button
                                    id="btn-cancel"
                                    type="button"
                                    class="hidden px-5 py-3 border border-slate-300 text-slate-700 rounded-xl font-button hover:bg-slate-50 transition-colors">
                                    Batal
                                </button>
                            </div>
                        </form>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            editButtons.forEach(button => {
                button.addEventListener('click', function () {
                    ...
                    const urutanInput = document.querySelector('input[name="urutan"]');
                    ...
                    if (isActiveCheckbox) {
                        isActiveCheckbox.checked = parseInt(isActive) === 1;
                    }

                    // Modify form target and method
                    form.action = `/admin/kategori/${id}`;
                    formMethod.value = 'PUT';
                    formTitle.textContent = 'Edit Kategori';
                    btnSubmit.textContent = 'Perbarui Kategori';
            ...
            if (btnCancel) {
                btnCancel.addEventListener('click', function () {
                    ...
                    // Reset fields manually to clear textareas and inputs not affected by reset
                    namaInput.value = '';
                    slugInput.value = '';
                    
                    const iconSelect = document.querySelector('select[name="icon"]');
                    if (iconSelect) iconSelect.value = 'category';
                    
                    const urutanInput = document.querySelector('input[name="urutan"]');
                    if (urutanInput) urutanInput.value = '1';
                    ...
                    formTitle.textContent = 'Form Kategori';
                    btnSubmit.textContent = 'Simpan Kategori';
